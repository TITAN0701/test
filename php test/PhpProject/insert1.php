<?php
	...
	mysql_select_db($dbname);   // 選擇 剛設定的$dbname 裡的myDB資料庫
 	$sql_insert = "INSERT INTO worktable (Id,Name,Count,Age,Email) VALUES ('$Id','$Name','$Count','$Age','$Email')";  //設定SQL指令
	$result = mysql_query($sql_insert) or die('MySQL insert error');  // 如果沒設定成功的SQL指令則跳出MySQL insert error的錯誤                   
	
	$sql_query = "select * from worktable"; //設定抓取的資料表 
	$result = mysql_query($sql_query) or die('MySQL worktable Id error'); // 如果設定的資料表沒抓取到則跳出 MySQL mysecond error的錯誤
